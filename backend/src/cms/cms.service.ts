...
export class CmsService {
  ...
  private removeLocalUpload(imageUrl: unknown) { if(typeof imageUrl!=='string') return; const filename=basename(new URL(imageUrl,'http://local').pathname); const path=join(process.cwd(),'uploads',filename); if(/^[a-f0-9-]+\.(jpe?g|png|webp)$/i.test(filename) && existsSync(path)) unlinkSync(path); }
  async update(kind: 'banners'|'faqs'|'socials', id: string, input: Record<string, unknown>) { const previous=await this.model(kind).findById(id).lean(); if(!previous) throw new NotFoundException('CMS item not found'); const item=await this.model(kind).findByIdAndUpdate(id,{$set:this.payload(kind,input)},{new:true,runValidators:true}).lean(); if(kind==='banners' && previous.imageUrl!==item?.imageUrl) this.removeLocalUpload(previous.imageUrl); return item; }
  async remove(kind: 'banners'|'faqs'|'socials', id: string) { const item=await this.model(kind).findByIdAndDelete(id).lean(); if(!item) throw new NotFoundException('CMS item not found'); if(kind==='banners') this.removeLocalUpload(item.imageUrl); return {deleted:true}; }
